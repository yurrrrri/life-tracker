...
import { Settings, Profile, Category, Todo, Journal, FontType } from '@/types'
import { STORAGE_KEYS } from '@/constants'

// Auth state
export const isAuthenticatedAtom = atom<boolean>(false)
export const authTokenAtom = atomWithStorage<string | null>(STORAGE_KEYS.AUTH_TOKEN, null)
...
export const isDarkModeAtom = atomWithStorage<boolean>(STORAGE_KEYS.THEME_MODE, false)
...
export const categoriesByDateAtom = atom((get) => {
  ...
  return categories.filter(category => 
    todoCategories.includes(category.id) && !category.removed
  )
})
